...
| Q2 | `GameController`'s widget-framework slice (mouse routing, hover/focus, `notify_all_widgets`, widget-file deserialization) keeps growing across three matrices now (`matrix-items.md`, `matrix-quests.md`, here) with no owning matrix or brick. Should `matrix-client-server.md` (028) absorb a `GameController` scoping section, or does it need a dedicated brick inserted before 224? | 224–231, 028 | brick 028 scoping decision — same question as `matrix-items.md` §4 Q1, now with UI-framework evidence added |
