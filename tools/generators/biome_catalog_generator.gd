...
## id -> [display_name, debug_color, surface_block_id, subsurface_block_id]. One row per
## `BiomeClassifier.IDS` entry, no more. A method rather than a `const`, because `Color8()`
## is a call and not a constant expression.
##
