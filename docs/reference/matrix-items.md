...
|---|---|---|---|
| Q1 | `cube::GameController` (620 functions, `cube/control/`) has no planned matrix among 021–028, yet several item/inventory functions — and, per `attribution.tsv`, the *actual* inventory-grid rebuild/scroll/hover logic that `GAP_ANALYSIS.md` mis-names `InventoryWidget_*` — live on it. Does a client-control matrix belong before brick 224/225, or does `matrix-client-server.md` (028) absorb it? | 224, 225 | brick 028 scoping decision, or a new matrix brick inserted before 224 |
