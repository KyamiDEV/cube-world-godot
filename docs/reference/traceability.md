...
| 061–063, 080, 081, 089–090 | `terrain-base-height-field.md` | how the original stacked that primitive into a ground height: three decade-spaced relief tiers, each *placed* by a squared weight field one decade coarser, added upward on a base blended from region data | `world/generation/elevation_field.gd`, `world/generation/erosion_pass.gd`, `world/generation/water_level.gd`, `world/generation/river_pass.gd`, `docs/world-generation.md` §6-7, §19-20 (brick 063's `terrace_pass.gd` inherits claim 6's "a pass only lowers" shape and nothing else — the quantisation itself has **no** reference basis, `docs/world-generation.md` §8.6; brick 064 **contradicted** the note's claim 7 and closed its `U2` — see the `terrain-climate-blend.md` row above; brick 080 read claim 6's own `World_waterDepthField` in full — claim 10, `LOW` — and found a chunk-type temperature modulation with no recovered return value, not a water-level computation, so `WaterLevel` is a clean-room constant plane with nothing to diverge from; brick 081 followed claim 6's own `World_riverClimateGate` shape — a per-column noise gate, not a flow simulation — without opening the helper body, and diverged on where the clip sits: it carves a whole terrace after quantisation instead of softening relief before it, `docs/world-generation.md` §20.6) | MEDIUM | — |
| 064–067, 074, 085, 089–090 | `terrain-climate-blend.md` | how the original produced climate: a nearest-region-site blend over stored per-region values, sharing only the site-jitter noise with the height field — so climate is **not** derived from elevation (closes `terrain-base-height-field.md` `U2`, contradicts its claim 7) | `world/generation/temperature_field.gd`, `world/generation/humidity_field.gd`, `world/biomes/biome_classifier.gd`, `docs/world-generation.md` §9-11 (brick 066 also took the note's claim 5 literals, `< 0.2` / `> 0.8`, as its own climate thresholds — the one piece of the original's *classification* that survived the read; brick 085 read a different function outside this note entirely — `GAP_ANALYSIS.md`'s `terrain_surfaceColor_blend`, `0x005c56e0`, `cube/game_misc/game_misc.cpp` — and found snow gated on **height and slope**, with no moisture term despite the summary line claiming one; kept the height-only idea, independently reached first in §9.3/§9.7, diverged on slope (already `ErosionPass.ruggedness_at()`'s question, one layer upstream) and on having any discrete material at all, `docs/world-generation.md` §24.6) | MEDIUM | — |
...
| 089–090 | `matrix-world.md` §2 | region-site & feature-cell placement (structure/POI seeding) — **both halves done**: 089 is one reproducible `StructureSeed` (anchor + owned sub-seed) per region (`srand(regX,regZ)` "one seed per region cell" shape kept, linear seed / global `rand()` dropped, `region-coordinate-hashing.md` §9); 090 is `is_placed_at(region)` — presence roll (`featureCountRange`/`featureTier`), `DecorationMask` eligibility, a one-terrace slope gate (the inverse of `objectFalloffWeight`), and an 8-neighbour spacing rule (`findNearestFeatureCell`/`falloffSquared`) that keeps placed structures ≥ 384 m apart world-wide. The relief *flattening* `objectFalloffWeight` describes is 091's | `world/structures/structure_seed_field.gd`, `world/structures/structure_placement.gd`, `docs/world-generation.md` §28–29 | MEDIUM | — |
| 090–091 | `matrix-world.md` §2 | `WorldInfo` structure placement — **090 and 091 both done**. 091 spent `objectFalloffWeight`/`falloffSquared`'s remaining half: `StructureGenerator.ground_falloff_at()` is a squared distance falloff levelling the ground under a placed structure, and `surface_y_at()` is the ground a generator fills to. Divergence: the original's falloff is a factor inside the relief product; ours is a pass **above** `TerracePass`, because 090's slope gate already reads it (a cycle), and it **cuts and fills** rather than only lowering — `docs/world-generation.md` §30.4. The `VoxelBuffer` write itself is **not** 091's: it belongs to inserted brick **091b**, §30.8 | `world/structures/structure_placement.gd`, `world/structures/structure_site.gd`, `world/structures/structure_generator.gd`, `docs/world-generation.md` §29–30 | MEDIUM | — |
...
- **Original design, no `cube::` behavior to draw from.** Phase A (001–010, bootstrap),
  ...
  *produced* the matrices, they don't consume one), brick 059 (Phase D's test fixtures —
  ...
  boundary to smooth in the first place; `docs/world-generation.md` §13.5), brick 075
  (surface material selection — the same finding a third time: no discrete biome means no
  discrete material either, so there is no material-selection mechanism to diverge from;
