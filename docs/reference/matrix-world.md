...
| `WorldInfo` biome content population (spawns, terrain features, decoration) | `WorldInfo_generateBiomeContent` (client) | `world/biomes/` | 067–068 | LOW |
...
| Procedural region/place naming (`NameGen::generateRegionName`, syllable tables) | client `World` section | `world/regions/` — no backlog brick currently covers this; flagged in §4 | — | LOW |
...
|---|---|---|---|
| Q1 | `ChunkBuffer_sampleVoxelColorAO` blends voxel color with baked ambient occlusion over a 3×3×3 neighborhood — does our chosen terrain material/shader need an equivalent, or is `VoxelMesherBlocky` baked-AO (if any) sufficient for the target look? | 040–041 | `world-terrain-material.md` (to be created at brick 040/041) |
| Q2 | The client runs its own copy of world generation (`WorldInfo`) rather than only presenting replicated server state. Confirm this was singleplayer/local-host convenience in the original game (not a networked-client trust model) before treating "client never generates" as safe to assume without caveat. | 056, 096–101 | `world-generation-authority.md` (to be created before brick 056) |
...
| Q4 | No backlog brick currently covers procedural region/place naming (`NameGen::generateRegionName`). Decide whether this is in scope at all (Phase D biome catalog, or a Phase J map/UI brick) or explicitly deferred. | — | next `nextsteps.md` update / backlog amendment |
